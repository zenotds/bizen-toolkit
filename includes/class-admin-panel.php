...
class Bizen_Admin_Panel {
	...
	public function handle_post(): void {
		...
		if ( isset( $_POST['bizen_check_now'] ) ) {
			...
			wp_safe_redirect( add_query_arg( [ 'page' => 'bizen-toolkit', 'checked' => '1' ], admin_url( 'admin.php' ) ) );
			exit;
		}
		...
		wp_safe_redirect( add_query_arg( [ 'page' => 'bizen-toolkit', 'saved' => '1' ], admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function render_page(): void {
		...
		<div class="wrap">
			<h1 style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
				...
				<span style="font-size:20px;font-weight:600;color:#1d2327;line-height:1;">Toolkit</span>
				<span style="font-size:12px;font-weight:400;color:#aaa;line-height:1;">
			...
			<form method="post" action="">
				...
				<h2><?php esc_html_e( 'Modules', 'bizen-toolkit' ); ?></h2>
				...
				<h2><?php esc_html_e( 'Version Monitor', 'bizen-toolkit' ); ?></h2>
				<p style="color:#757575;">
					<?php esc_html_e( 'Weekly background check against the WordPress.org plugin API. When an upstream version is newer than the vendored one, a flag appears in the table above.', 'bizen-toolkit' ); ?>
				</p>
				<p>
					<?php if ( $next_check ) : ?>
						<?php printf(
				...
				<?php if ( $monitor ) : ?>
					<?php
					$last = max( array_column( $monitor, 'last_checked' ) );
					?>
